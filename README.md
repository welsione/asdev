...
```text
/asdev 处理这个任务：
net.qiyuesuo.cloud.cms.exception.CmsErrorCode#CERT_APPLY_ERROR
这个类型的报错抛出的异常要抛出到接口层面。
找到所有调用点和间接调用点，确保可以抛出到 Controller 接口层。
```

```text
/asdev 用多 Agent 工作流重构这个大 Service。
先调查当前代码和依赖，再产出需求探索和任务拆解。
